...
def draw_PBA_chart(PBA_model, m,e):
    
    axes[0].set_xlim([0,PBA_model.n_target])
    
    if "_LDP" in PBA_model.name_:
        axes[0].barh(str(e), PBA_model.TP ,hatch="///", align='center', color=color_LDP, zorder=10)
    ...
    axes[1].set_xlim([0,PBA_model.n_target])
    
    if "_DP" in PBA_model.name_:
        axes[1].barh(str(e), PBA_model.TP , align='center', color=color_DP, zorder=10)
...
if __name__=="__main__":
    ...
    for e in epsilons:
        for m in Models:
            with open("privacy_models/PBA/PBA_"+m+"_"+str(e) + ".pickle","rb") as fr:
                PBA_model = pickle.load(fr)
            draw_PBA_chart(PBA_model, m, e)
    plt.subplots_adjust(wspace=0, top=0.85, bottom=0.1, left=0.18, right=0.95)
