...
def draw_WDA_chart(WDA_model, m,e):
 
    axes[0].set_xlim([0,WDA_model.threshold])
    axes[0].axvline(WDA_model.threshold/2,color='black',ls='--',linewidth=1)   
    if "_LDP" in WDA_model.name_:
        axes[0].barh(str(e), WDA_model.TP ,hatch="///", align='center', color=color_LDP, zorder=10)
    ...
    axes[1].set_xlim([0,WDA_model.threshold])
    axes[1].axvline(WDA_model.threshold/2,color='black',ls='--',linewidth=1)
    if "_DP" in WDA_model.name_:

        axes[1].barh(str(e), WDA_model.TP , align='center', color=color_DP, zorder=10)
...
if __name__=="__main__":
    ...
    for e in epsilons:
        for m in Models:
            with open("privacy_models/WDA/WDA_"+m+"_"+str(e) + ".pickle","rb") as fr:
                WDA_model = pickle.load(fr)
            draw_WDA_chart(WDA_model, m, e)
    plt.subplots_adjust(wspace=0, top=0.85, bottom=0.1, left=0.18, right=0.95)
