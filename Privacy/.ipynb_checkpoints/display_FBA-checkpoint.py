...
def draw_FBA_chart(FBA_model, m,e):
    
    axes[0].set_xlim([0,FBA_model.n_target])
    
    if "_LDP" in FBA_model.name_:
        axes[0].barh(str(e), FBA_model.TP ,hatch="///", align='center', color=color_LDP, zorder=10)
    ...
    axes[1].set_xlim([0,FBA_model.n_target])
    
    if "_DP" in FBA_model.name_:
        axes[1].barh(str(e), FBA_model.TP , align='center', color=color_DP, zorder=10)
...
if __name__=="__main__":
    ...
    for e in epsilons:
        for m in Models:
            with open("privacy_models/FBA/FBA_"+m+"_"+str(e) + ".pickle","rb") as fr:
                FBA_model = pickle.load(fr)
            draw_FBA_chart(FBA_model, m, e)
