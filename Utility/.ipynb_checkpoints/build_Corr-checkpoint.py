...
if __name__=="__main__":
    ...
    for m in Models:
        for e in epsilons:
            ...
            Corr_model = Corr(name, n_data,columns, bin_cols)
            Corr_model.real_score = get_Corr(real_data)

            syn_data =  generate_syn_data(generator, bin_cols, n_data, columns, round_dict, latent_dim) 

            Corr_model.syn_score = get_Corr(syn_data)
            
            Corr_model.get_distance(Corr_model.real_score, Corr_model.syn_score)
            
            with open("utility_models/Corr/{}.pickle".format(Corr_model.name_),"wb") as fw:
                pickle.dump(Corr_model, fw)